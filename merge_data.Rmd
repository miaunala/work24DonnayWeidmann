...
joined_data <- joined_data %>% group_by(year, org_id_from_coll) %>%
  mutate(
    post_count = n(),                              # Count of posts per group
    global_posts = sum(global == 1),               # Count of global posts per group
    prop_global = global_posts / post_count        # Proportion of global posts per group
  ) %>%
  ungroup()
```
